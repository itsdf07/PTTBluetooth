package com.itsdf07.pttbluetooth.gaiabyroen;
...
public class GaiaLink {
     ...
     *
     * @param dataConnectionListener An object that implements the DataConnectionListener
     *                               interface.
     */
    ...
    // Gateway address from the emulator will always be 10.0.2.2; the port might
    // change if
    // we need to support multiple instances. You need to redirect the host port
    // like so:
     ...
     *
     * @param address Bluetooth or IP address of the remote device depending on the
     *                transport.
     * @throws IOException
     */
    ...
    /**
     * Set the debug level which controls how verbose we are with debugging
     * information sent to logcat.
     *
     * @param level The debug level. Set to zero for no debug logging, 1 for
     *              standard debug messages, 2 for verbose debug messages.
     */
    ...
    public void sendCommand(int vendor_id, int command_id, int... param) throws IOException {
        if (param == null || param.length == 0) {
            ...
            // Convenient but involves copying the payload twice. It's usually
            // short.
            byte[] payload;
    ...
    /**
     * Sends a Gaia acknowledgement to the remote device, assuming the vendor id
     * is CSR
     *
     * @param command_id The command identifier.
     * @param status     The status of the command.
    ...
    public void sendAcknowledgement(GaiaCommand command, Gaia.Status status) throws IOException {
        sendAcknowledgement(command.getVendorId(), command.getCommandId(), status);
    }

    public void sendAcknowledgement(GaiaCommand command, Gaia.Status status, int... payload) throws IOException {
        sendAcknowledgement(command.getVendorId(), command.getCommandId(), status, payload);
    }

    /**
     * Requests the device to perform no operation (other than to send an
    ...
    /**
     * Requests the current Received Signal Strength Indication at the remote
     * device.
     *
     * @throws IOException
     */
    ...
    /**
     * Requests the Application Version of the remote device. The
     * acknowledgement payload contains an eight-octet application version
     * identifier optionally followed by null-terminated human-readable text.
     *
     * @throws IOException
     */
     ...
     *
     * @param handler The Handler for debugging messages received from the remote
     *                device.
     */
    ...
    /**
     * Log a hex string representation of a commands payload bytes to the log
     * handler.
     *
    ...
    /**
     * Thread to connect the Bluetooth socket and start the thread that reads
     * from the socket.
     */
        ...
        private void runSppReader() {
            ...
            if (mIsListening) {
                try {
                    ...
                    mReceiveHandler.obtainMessage(Message.ERROR.ordinal(), e).sendToTarget();
                    System.out.println("Message.What:" + Message.ERROR.ordinal() + "ERROR");
                    going = false;
                }
            } else {
                mReceiveHandler.obtainMessage(Message.CONNECTED.ordinal(), mBTDevice.getAddress()).sendToTarget();
                mIsConnected = true;
                going = true;
                System.out.println("Message.What:" + Message.CONNECTED.ordinal() + "CONNECT");
            }
     ...
     * search handler which should have been registered via
     * setDeviceSearchHandler() before calling this method.
     */
